...
def calculate_molecule_3D_structure():
    ...
    mutex = Lock()
    os.system('rm ../data/result/invalid_smiles.txt')
...
class UniMolModel(nn.Module):
    ...
    def forward(
        ...
        src_tokens, src_distance, src_coord, src_edge_type = net_input['src_tokens'], net_input['src_distance'], net_input['src_coord'], net_input['src_edge_type']
...
class EsmUnimolClassifier(nn.Module):
    def __init__(self):
        ...
        self.molecule_encoder.load_state_dict(th.load('../weight/mol_pre_no_h_220816.pt')['model'], strict=False)
    ...
    def forward(self, data_batch):
        ...
        # protein_padding_mask = (token_batch == self.alphabet.padding_idx)
        protein_encoder_output = self.protein_encoder(token_batch, repr_layers=[33], return_contacts=False)
        protein_enbedding = protein_encoder_output["representations"][33]
    
        # x = self.transformer_layer_cross_attention(molecule_embedding, protein_enbedding, protein_padding_mask)
        x = self.transformer_layer_cross_attention(molecule_embedding, protein_enbedding, None)
...
def evaluate(model, data_loader):
    ...
    metric = {'auc_roc': auc_roc, 'auc_prc': auc_prc, 'accuracy': accuracy, 'precision': precision, 'recall': recall, 'f1_score': f1_score}
    return metric
...
def train(trial_version):
    ...
    for name, p in model.protein_encoder.named_parameters():
        if name.startswith('layers.30') or name.startswith('layers.31') or name.startswith('layers.32') \
            or name.startswith('layers.27') or name.startswith('layers.28') or name.startswith('layers.29'):
            # or name.startswith('layers.24') or name.startswith('layers.25') or name.startswith('layers.26') \
            # or name.startswith('layers.21') or name.startswith('layers.22') or name.startswith('layers.23'):
    ...
    for epoch in range(200):
        ...
        # for step, data_batch in enumerate(tqdm(data_loader_train)):
            label_predict_batch = model(data_batch)
            label_true_batch = data_batch['target']['label'].to(th.long)
        ...
        if metric_validate['auc_prc'] > current_best_metric:
            current_best_metric = metric_validate['auc_prc']
